...
export default function ControlBar({ style = {}, options }: ControlBarProps) {
  ...
  return <NativeControlBar style={style} options={options} />;
}

function NativeControlBar({ style = {}, options }: ControlBarProps) {
  const { useLocalParticipant } = require('@livekit/components-react');
  const { BarVisualizer } = require('@livekit/react-native');

  const { microphoneTrack, localParticipant } = useLocalParticipant();
  const [trackRef, setTrackRef] = useState<any | undefined>(undefined);
